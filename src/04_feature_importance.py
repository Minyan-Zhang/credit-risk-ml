"""
04_feature_importance.py — 特征重要性分析
==========================================
运行方式：python src/04_feature_importance.py
输出：outputs/ 目录下的特征重要性图表
"""
...
def plot_xgboost_importance(features: list):
    ...
    bars = ax.barh(feat_imp["label"], feat_imp["importance"],
                   color=plt.cm.RdYlGn(feat_imp["importance"] / feat_imp["importance"].max()))
...
def plot_random_forest_importance(features: list):
    ...
    ax.barh(feat_imp["label"], feat_imp["importance"],
            color=plt.cm.Blues(feat_imp["importance"] / feat_imp["importance"].max() * 0.7 + 0.3))
...
def plot_logistic_coefficients(features: list):
    """Logistic Regression 系数（标准化后代表相对重要性）"""
